<?php
	...
	for($i = 0; $i < count($xmlFile->table); $i++) {
		...
		$json_node = $xmlFile->table[$i]->plugins->summary_reports->report_details ?? null;
...
<div class="row">
	...
	$filterable_fields = ' . json_encode($filterable_fields) . ';'; 
		...
		'override_permissions' => " . (($reports[$j]->override_permissions ?? false) ? 'true' : 'false') . ",
		...
		'jsmoment_date_format' => '" . strtoupper($date_formats[$date_format]) . "'";
							...
							if(!isset($reports[$j]->parent_table)) {
								...
								if($reports[$j]->look_up_table ?? null) {								
							...
							if($reports[$j]->report_header_url ?? null) {	
								$generated_report_content .= ",\n\t\t'report_header_url' => '" . addslashes($reports[$j]->report_header_url) . "'";
							}
							if($reports[$j]->report_footer_url ?? null) {
