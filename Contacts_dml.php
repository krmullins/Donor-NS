...
function Contacts_insert(&$error_message = '') {
	global $Translation;
	...
	if(function_exists('Contacts_before_insert')) {
		$args = [];
		if(!Contacts_before_insert($data, getMemberInfo(), $args)) {
	...
	update_calc_fields('Contacts', $recID, calculated_fields()['Contacts']);
	...
	set_record_owner('Contacts', $recID, $recordOwner);

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID'))) Contacts_copy_children($recID, Request::val('SelectedID'));
...
function Contacts_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	if(!check_record_permission('Contacts', $selected_id, 'delete')) {
	...
	$ID = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `Donations` WHERE `ContactID`='" . makeSafe($ID[0]) . "'", $eo);
	...
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		...
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
		$RetMsg = str_replace('<TableName>', 'Donations', $RetMsg);
		$RetMsg = str_replace('<Delete>', '<input type="button" class="btn btn-danger" value="' . html_attr($Translation['yes']) . '" onClick="window.location = \'Contacts_view.php?SelectedID=' . urlencode($selected_id) . '&delete_x=1&confirmed=1&csrf_token=' . urlencode(csrf_token(false, true)) . '\';">', $RetMsg);
		$RetMsg = str_replace('<Cancel>', '<input type="button" class="btn btn-success" value="' . html_attr($Translation[ 'no']) . '" onClick="window.location = \'Contacts_view.php?SelectedID=' . urlencode($selected_id) . '\';">', $RetMsg);
		return $RetMsg;
	}
	...
	$res = sql("SELECT `ID` FROM `Contacts` WHERE `ID`='{$selected_id}'", $eo);
	$ID = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `Bidders` WHERE `ContactID`='" . makeSafe($ID[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
		$RetMsg = str_replace('<TableName>', 'Bidders', $RetMsg);
		return $RetMsg;
	} elseif($rirow[0] && $AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation['confirm delete'];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
...
function Contacts_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('Contacts', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('Contacts', $selected_id);
	...
	if(function_exists('Contacts_before_update')) {
		...
		if(!Contacts_before_update($data, getMemberInfo(), $args)) {
			if(isset($args['error_message'])) $error_message = $args['error_message'];
			return false;
		}
	}
	...
	update_calc_fields('Contacts', $data['selectedID'], calculated_fields()['Contacts']);
	...
	set_record_owner('Contacts', $selected_id);
}

function Contacts_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $separateDV = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if(is_file(__DIR__ . '/hooks/Contacts.State.csv')) {
		$State_data = addslashes(implode('', @file(__DIR__ . '/hooks/Contacts.State.csv')));
	...
	if(is_file(__DIR__ . '/hooks/Contacts.Status.csv')) {
		$Status_data = addslashes(implode('', @file(__DIR__ . '/hooks/Contacts.Status.csv')));
	...
	if(is_file(__DIR__ . '/hooks/Contacts.ContactMethod.csv')) {
		$ContactMethod_data = addslashes(implode('', @file(__DIR__ . '/hooks/Contacts.ContactMethod.csv')));
		...
		$combo_ContactMethod->ListItem = array_trim(explode('||', entitiesToUTF8(convertLegacyOptions("Email;;Cell;;Phone"))));
		$combo_ContactMethod->ListData = $combo_ContactMethod->ListItem;
	}
	$combo_ContactMethod->SelectName = 'ContactMethod';

	if($selected_id) {
		if(!check_record_permission('Contacts', $selected_id, 'view'))
			return $Translation['tableAccessDenied'];

		// can edit?
		$AllowUpdate = check_record_permission('Contacts', $selected_id, 'edit');

		// can delete?
		$AllowDelete = check_record_permission('Contacts', $selected_id, 'delete');
	...
	$templateCode .= loadView('Contacts-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);
