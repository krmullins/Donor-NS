<?php
	...
	*/
	...
	function get_table_groups($skip_authentication = false) {
		...
		$all_groups = ['Event', 'Auction', 'Registration', 'Settings'];
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false, $lower_permissions = false) {
		$sql_from = [
			'Contacts' => "`Contacts` ",
			'Settings' => "`Settings` ",
			'Donations' => "`Donations` LEFT JOIN `Catalog` as Catalog1 ON `Catalog1`.`ID`=`Donations`.`CatalogID` LEFT JOIN `Contacts` as Contacts1 ON `Contacts1`.`ID`=`Donations`.`ContactID` ",
			'Catalog' => "`Catalog` LEFT JOIN `CatalogTypes` as CatalogTypes1 ON `CatalogTypes1`.`ID`=`Catalog`.`TypeID` LEFT JOIN `CatalogGroups` as CatalogGroups1 ON `CatalogGroups1`.`ID`=`Catalog`.`GroupID` ",
			'Bidders' => "`Bidders` LEFT JOIN `Contacts` as Contacts1 ON `Contacts1`.`ID`=`Bidders`.`ContactID` ",
			'Transactions' => "`Transactions` LEFT JOIN `Catalog` as Catalog1 ON `Catalog1`.`ID`=`Transactions`.`CatalogID` LEFT JOIN `Bidders` as Bidders1 ON `Bidders1`.`ID`=`Transactions`.`BidderID` LEFT JOIN `Contacts` as Contacts1 ON `Contacts1`.`ID`=`Bidders1`.`ContactID` ",
			'Payments' => "`Payments` LEFT JOIN `Bidders` as Bidders1 ON `Bidders1`.`ID`=`Payments`.`BidderID` LEFT JOIN `Contacts` as Contacts1 ON `Contacts1`.`ID`=`Bidders1`.`ContactID` ",
		...
		$pcConfig = [
			...
			'Donations' => [
				'CatalogID' => [
					...
					'tab-label' => 'Donations <span class="hidden child-label-Donations child-field-caption">(Catalog Number)</span>',
					'auto-close' => true,
					'table-icon' => 'resources/table_icons/32Px - 396.png',
					'display-refresh' => true,
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => [1 => 'Donation Name', 2 => 'Description', 3 => 'Restrictions', 4 => 'Value', 5 => 'Catalog Number', 6 => 'Donor Name Lookup', 7 => 'Contact Person', 8 => 'Contact Phone', 9 => 'Item Status', 10 => 'Procured By', 11 => 'Date Procured', 12 => 'Additional Info', 13 => 'Thanks', 14 => 'Notes'],
					'display-field-names' => [1 => 'DonationName', 2 => 'Description', 3 => 'Restrictions', 4 => 'Value', 5 => 'CatalogID', 6 => 'ContactID', 7 => 'ContactPerson', 8 => 'ContactPhone', 9 => 'ItemStatus', 10 => 'ProcuredBy', 11 => 'DateProcured', 12 => 'AdditionalInfo', 13 => 'Thanks', 14 => 'Notes'],
					'sortable-fields' => [0 => '`Donations`.`ID`', 1 => 2, 2 => 3, 3 => 4, 4 => '`Donations`.`Value`', 5 => 6, 6 => '`Contacts1`.`MailingNameFull`', 7 => 8, 8 => 9, 9 => 10, 10 => 11, 11 => '`Donations`.`DateProcured`', 12 => 13, 13 => 14, 14 => 15],
					'records-per-page' => 10,
					...
					'query' => "SELECT `Donations`.`ID` as 'ID', `Donations`.`DonationName` as 'DonationName', `Donations`.`Description` as 'Description', `Donations`.`Restrictions` as 'Restrictions', `Donations`.`Value` as 'Value', IF(    CHAR_LENGTH(`Catalog1`.`CatalogNo`) || CHAR_LENGTH(`Catalog1`.`CatalogTitle`), CONCAT_WS('',   `Catalog1`.`CatalogNo`, ' - ', `Catalog1`.`CatalogTitle`), '') as 'CatalogID', IF(    CHAR_LENGTH(`Contacts1`.`MailingNameFull`), CONCAT_WS('',   `Contacts1`.`MailingNameFull`), '') as 'ContactID', `Donations`.`ContactPerson` as 'ContactPerson', `Donations`.`ContactPhone` as 'ContactPhone', `Donations`.`ItemStatus` as 'ItemStatus', `Donations`.`ProcuredBy` as 'ProcuredBy', if(`Donations`.`DateProcured`,date_format(`Donations`.`DateProcured`,'%m/%d/%Y'),'') as 'DateProcured', `Donations`.`AdditionalInfo` as 'AdditionalInfo', `Donations`.`Thanks` as 'Thanks', `Donations`.`Notes` as 'Notes' FROM `Donations` LEFT JOIN `Catalog` as Catalog1 ON `Catalog1`.`ID`=`Donations`.`CatalogID` LEFT JOIN `Contacts` as Contacts1 ON `Contacts1`.`ID`=`Donations`.`ContactID` "
				],
				'ContactID' => [
					...
					'child-primary-key' => 'ID',
					'child-primary-key-index' => 0,
					'tab-label' => 'Donations <span class="hidden child-label-Donations child-field-caption">(Donor Name Lookup)</span>',
					'auto-close' => true,
					'table-icon' => 'resources/table_icons/32Px - 396.png',
					'display-refresh' => true,
					'display-add-new' => true,
			...
			'Transactions' => [
				'CatalogID' => [
					...
					'child-primary-key' => 'ID',
					'child-primary-key-index' => 0,
					'tab-label' => 'Transactions <span class="hidden child-label-Transactions child-field-caption">(Catalog #)</span>',
					'auto-close' => true,
					'table-icon' => 'resources/table_icons/32Px - 397.png',
					'display-refresh' => true,
					'display-add-new' => true,
					'forced-where' => '',
				...
				'BidderID' => [
					...
					'child-primary-key' => 'ID',
					'child-primary-key-index' => 0,
					'tab-label' => 'Transactions <span class="hidden child-label-Transactions child-field-caption">(Bidder)</span>',
					'auto-close' => true,
					'table-icon' => 'resources/table_icons/32Px - 397.png',
					'display-refresh' => true,
					'display-add-new' => true,
			...
			'Payments' => [
				'BidderID' => [
					...
					'child-primary-key' => 'ID',
					'child-primary-key-index' => 0,
					'tab-label' => 'Payments <span class="hidden child-label-Payments child-field-caption">(Bidder)</span>',
					'auto-close' => true,
					'table-icon' => 'resources/table_icons/32Px - 385.png',
					'display-refresh' => true,
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => [1 => 'Date', 2 => 'Bidder', 3 => 'Payment Amount', 4 => 'Payment Type'],
					'display-field-names' => [1 => 'Date', 2 => 'BidderID', 3 => 'PaymentAmount', 4 => 'PaymentType'],
					'sortable-fields' => [0 => '`Payments`.`ID`', 1 => '`Payments`.`Date`', 2 => 3, 3 => '`Payments`.`PaymentAmount`', 4 => 5],
					'records-per-page' => 10,
					'default-sort-by' => 2,
					'default-sort-direction' => 'asc',
					'open-detail-view-on-click' => true,
					...
					'query' => "SELECT `Payments`.`ID` as 'ID', if(`Payments`.`Date`,date_format(`Payments`.`Date`,'%m/%d/%Y'),'') as 'Date', IF(    CHAR_LENGTH(`Bidders1`.`BidNo`) || CHAR_LENGTH(`Contacts1`.`MailingNameFull`), CONCAT_WS('',   `Bidders1`.`BidNo`, ' - ', `Contacts1`.`MailingNameFull`), '') as 'BidderID', CONCAT('$', FORMAT(`Payments`.`PaymentAmount`, 2)) as 'PaymentAmount', `Payments`.`PaymentType` as 'PaymentType' FROM `Payments` LEFT JOIN `Bidders` as Bidders1 ON `Bidders1`.`ID`=`Payments`.`BidderID` LEFT JOIN `Contacts` as Contacts1 ON `Contacts1`.`ID`=`Bidders1`.`ContactID` "
				],
			],
			'CatalogTypes' => [
			],
			...
			'Tickets' => [
				'BidderID' => [
					...
					'child-primary-key' => 'ID',
					'child-primary-key-index' => 0,
					'tab-label' => 'Tickets <span class="hidden child-label-Tickets child-field-caption">(Bidder Number)</span>',
					'auto-close' => true,
					'table-icon' => 'resources/table_icons/32Px - 394.png',
					'display-refresh' => true,
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => [1 => 'Guest Name', 2 => 'Bidder Number', 3 => 'Table Preference', 4 => 'Table', 5 => 'Table Name', 6 => 'Seating Position'],
					'display-field-names' => [1 => 'UsersName', 2 => 'BidderID', 3 => 'TablePreference', 4 => 'TableID', 5 => 'TableName', 6 => 'SeatingPosition'],
					'sortable-fields' => [0 => '`Tickets`.`ID`', 1 => 2, 2 => '`Bidders1`.`BidNo`', 3 => '`Bidders1`.`TablePreference`', 4 => 5, 5 => '`Tables1`.`TableName`', 6 => 7],
					'records-per-page' => 10,
					'default-sort-by' => false,
					'default-sort-direction' => 'asc',
					'open-detail-view-on-click' => true,
					'display-page-selector' => true,
					'show-page-progress' => true,
					'template' => 'children-Tickets',
					'template-printable' => 'children-Tickets-printable',
					'query' => "SELECT `Tickets`.`ID` as 'ID', `Tickets`.`UsersName` as 'UsersName', IF(    CHAR_LENGTH(`Bidders1`.`BidNo`), CONCAT_WS('',   `Bidders1`.`BidNo`), '') as 'BidderID', IF(    CHAR_LENGTH(`Bidders1`.`TablePreference`), CONCAT_WS('',   `Bidders1`.`TablePreference`), '') as 'TablePreference', IF(    CHAR_LENGTH(`Tables1`.`TableNo`) || CHAR_LENGTH(`Tables1`.`TableName`), CONCAT_WS('',   `Tables1`.`TableNo`, ' - ', `Tables1`.`TableName`), '') as 'TableID', IF(    CHAR_LENGTH(`Tables1`.`TableName`), CONCAT_WS('',   `Tables1`.`TableName`), '') as 'TableName', `Tickets`.`SeatingPosition` as 'SeatingPosition' FROM `Tickets` LEFT JOIN `Bidders` as Bidders1 ON `Bidders1`.`ID`=`Tickets`.`BidderID` LEFT JOIN `Tables` as Tables1 ON `Tables1`.`ID`=`Tickets`.`TableID` "
				],
				'TableID' => [
					'parent-table' => 'Tables',
					'parent-primary-key' => 'ID',
					'child-primary-key' => 'ID',
					'child-primary-key-index' => 0,
					'tab-label' => 'Tickets <span class="hidden child-label-Tickets child-field-caption">(Table)</span>',
					'auto-close' => true,
					'table-icon' => 'resources/table_icons/32Px - 394.png',
					'display-refresh' => true,
					'display-add-new' => true,
